...
GAMEFU_API gfuobj_raw* gfuobj_builder_to_raw(gfuobj_builder* builder) {
    size_t raw_size = sizeof(gfuobj_header) + (sizeof(gfuobj_section) * builder->count);
    ...
    for (size_t i = 0; i < builder->count; raw_size = GFUOBJ_ALIGN(raw_size), i++) {
    ...
    header->flags = builder->flags;
    header->entry_address = builder->entry_address;
    header->rom_size = raw_size;

    size_t current_offset = sizeof(gfuobj_header) + (sizeof(gfuobj_section) * builder->count);
    for (size_t i = 0; i < builder->count; current_offset = GFUOBJ_ALIGN(current_offset), i++) {
        ...
        *section_header = (gfuobj_section) {
        ...
        if (i == GFUOBJ_SYMBOL_SECTIDX) {
            ...
            for (size_t j = 0, j_count = section->data.count / sizeof(gfuobj_symbol_builder); j < j_count; j++) {
...
GAMEFU_API gfuobj_addr gfuobj_builder_intern_string(gfuobj_builder* builder, const char* string) {
    ...
    size_t string_addr = strdat_builder->count;
...
GAMEFU_API gfuobj_symidx gfuobj_byte_builder_push_symbol(gfuobj_byte_builder* byte_builder) {
    ...
    gfuobj_symidx index = byte_builder->count / sizeof(gfuobj_symbol);

    gfu_da_reserve(byte_builder, byte_builder->count + sizeof(gfuobj_symbol));
    ((gfuobj_symbol*)byte_builder->items)[index] = (gfuobj_symbol) {0};

    byte_builder->count += sizeof(gfuobj_symbol);
    return index;
}

GAMEFU_API gfuobj_relidx gfuobj_byte_builder_push_relocation(gfuobj_byte_builder* byte_builder) {
    if (byte_builder == nullptr) {
        return GFUOBJ_RELIDX_INVALID;
    }
